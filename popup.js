...
async function fillPassword() {
  ...
  try {
    ...
    await addHistoryItem(password, 'fill', getTabUrl(tab));
...
function renderCopyHistory() {
  fields.copyHistory.replaceChildren(
    ...copyHistory.map((item) => {
      ...
      historyItem.append(meta, value, url);
      return historyItem;
    }),
  );
}
...
async function addHistoryItem(value, action = 'copy', url = '') {
  copyHistory = [{ value, action, url, copiedAt: new Date().toISOString() }, ...copyHistory].slice(0, HISTORY_LIMIT);
  renderCopyHistory();
